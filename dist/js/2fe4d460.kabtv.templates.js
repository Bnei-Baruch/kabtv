angular.module('kabtv').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/layout/footer.html',
    "<div data-ng-controller=\"Footer as vm\"><nav class=\"links-nav hidden-sm hidden-xs\"><a data-ng-href={{item.url}} data-ng-repeat=\"item in vm.links\" class=nav-link>{{item.caption}}</a></nav></div>"
  );


  $templateCache.put('app/layout/header.html',
    "<div id=header data-ng-controller=\"Header as vm\"><nav class=\"navbar navbar-default\"><div class=container-fluid><div class=navbar-header data-ng-init=\"vm.topNavLang = true\"><a class=navbar-brand href=#><div class=\"tree toL\"><img src=static/images/tree.gif></div><span class=toL><img data-ng-src={{vm.logoImage}}></span></a> <button type=button class=\"navbar-toggle collapsed\" data-ng-click=\"vm.topNavLang = !vm.topNavLang\"><span class=sr-only>Toggle navigation</span> <span class=icon-bar></span> <span class=icon-bar></span> <span class=icon-bar></span></button></div><div class=white-wrappwer data-ng-show=vm.topNavLang><nav class=\"nav langs-nav hidden-sm hidden-xs\"><a data-ng-repeat=\"item in vm.languages\" data-ng-href=/{{item.key|lowercase}} data-ng-class=\"{'nav-link': true, active: vm.isCurrentLang(item)}\">{{item.caption}}</a><div id=tech-support class=\"navbar-right hidden-sm hidden-xs\"><a href=\"\" data-ng-click=vm.gotoHelpMessage()><img data-ng-src={{vm.helpImage}} alt=\"technical support\" border=0 width=226 height=\"50\"></a></div></nav></div></div></nav><nav class=\"links-nav hidden-sm hidden-xs\"><a data-ng-repeat=\"item in vm.links\" data-ng-href={{item.url}} class=nav-link title={{item.title}}>{{item.caption}}</a></nav></div>"
  );


  $templateCache.put('app/layout/shell.html',
    "<div id=shell class=\"container {{vm.pageDirection}}\" data-ng-controller=\"Shell as vm\"><header class=clearfix><div data-ng-include=\"'app/layout/header.html'\"></div></header><div id=main-panel class=\"clearfix row\"><aside data-ng-include=\"'app/vod/vod.html'\" class=\"col-md-3 hidden-xs hidden-sm aside\"></aside><div data-ng-view=\"\" class=\"col-md-6 col-sm-8 center-panel\">Center panel content</div><aside data-ng-include=\"'app/tabs/tabs.html'\" class=\"col-md-3 col-sm-4 aside\"></aside></div><footer id=footer class=\"clearfix modal-footer navbar-fixed-bottom\"><div data-ng-include=\"'app/layout/footer.html'\"></div></footer></div>"
  );


  $templateCache.put('app/player/clip.html',
    "<div class=clearfix><div class=\"tabs clearfix\"><div class=\"back-to-stream btn btn-xs btn-warning toL\" data-ng-click=vm.gotoStream() data-ng-if=\"$root.isLive || $root.config.lang.key == 'HEB' || $root.config.lang.key == 'RUS'\">{{ 'PLAYER_BACK_TO_LIVE' | translate}}</div></div><span us-spinner spinner-key=spinner-1 spinner-start-active=true></span><div id=player><kabtv-jw-player data-ng-if=!vm.isWMV data-file=vm.clip.play_url></kabtv-jw-player><kabtv-wmv-player data-ng-if=vm.isWMV data-url=vm.clip.play_url></kabtv-wmv-player></div><div class=player-panel><h4 class=clip-title>{{vm.clip.title}}</h4><div id=clip-save-button><a data-ng-href={{vm.clip.download_url}} class=\"download-icon clip-icon-big\"><img src=static/images/save-big.gif title=\"{{ 'VOD_DOWNLOAD' | translate}}\" alt=\"{{ 'VOD_DOWNLOAD' | translate}}\"> {{ 'VOD_DOWNLOAD' | translate}}</a></div><p>{{vm.clip.description}}</p><br><div id=addthis-toolbox class=\"addthis_toolbox addthis_default_style addthis_16x16_style\"><a class=addthis_button_twitter></a> <a class=addthis_button_google_plusone_share></a> <a class=addthis_button_email></a></div></div></div>"
  );


  $templateCache.put('app/player/jwplayer.directive.html',
    "<div id=player><div id=jwplayer-container class=\"embed-responsive embed-responsive-16by9\"></div></div>"
  );
  ...
  $templateCache.put('app/player/stream.html',
    "<div class=clearfix><ul class=\"tabs text-to-right clearfix\"><li role=presentation data-ng-class=\"{'tab-button': true, 'select': vm.isVideo}\" data-ng-click=vm.playVideo()>{{'PLAYER_VIDEO' | translate}}</li><li data-ng-if=vm.hasAudio role=presentation data-ng-click=vm.playAudio() data-ng-class=\"{'tab-button': true, 'select': !vm.isVideo}\">{{'PLAYER_AUDIO' | translate}}</li><li class=\"tab-right btn btn-xs btn-warning toL\" translate=PLAYER_BROADCAST_TIME translate-value-time={{vm.jerusalemTime}}></li></ul><div id=player><kabtv-jw-player data-ng-if=\"vm.isVideo && vm.isHLS\" data-stream=vm.stream></kabtv-jw-player><kabtv-sound-player data-ng-if=vm.isAudio data-url=vm.audioStream.url></kabtv-sound-player></div><div class=player-panel><div class=\"btn-group btn-group-xs\" role=group><label data-ng-repeat=\"item in vm.alternateLanguages\" class=\"langs btn btn-default\" role=toolbar data-ng-if=\"isLive || !item.is_live_only\">{{item.language.caption}} <input type=radio data-ng-model=vm.currentPlayerLang data-ng-value=item.language.key data-ng-change=vm.changeLanguage(item.language) name=\"vm.switcher-language-{{item.language.key}}\"></label></div><div class=quality-switcher data-ng-show=vm.showQualitySelection()><span>{{'STREAM_QUALITY' | translate}}:</span><label data-ng-repeat=\"item in vm.alternateQualities | orderBy: item.quality\" class=langs>{{item.quality.toUpperCase() | translate}} <input type=radio data-ng-model=vm.currentPlayerQuality value=item.quality name=vm.switcher-quality-{{item.quality}} data-ng-change=\"vm.changeQuality(item.quality)\"></label><label class=\"langs selected\">{{vm.currentPlayerQuality.toUpperCase() | translate}}</label></div></div></div>"
  );
  ...
  $templateCache.put('app/vod/vod.html',
    "<div id=vod data-ng-controller=\"Vod as vm\"><select id=vod-categories data-ng-model=vm.selectedCategory data-ng-change=vm.changeCategory() data-ng-options=\"item.name for item in vm.categories\"></select><div id=vod-items data-ng-style=vm.vodItemsStyle()><div data-ng-repeat=\"item in vm.vodItems\" class=vod-item><div data-ng-click=vm.playVod(item) class=clearfix><img data-ng-src={{item.image_url}} class=\"vod-item-image toL\" title={{item.title}} alt={{item.title}} width=70 height=\"70\"><h5 class=vod-item-title>{{item.title}}</h5><p class=vod-item-description>{{item.description}}</p><p class=vod-item-duration>{{item.duration}}</p></div></div></div></div>"
  );
